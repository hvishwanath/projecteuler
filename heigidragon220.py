...
def getDn_algo(n,start=0,dprev=None):
    """
    D1 = D0 + a
    D2 = D1 + RbFR  <b should expand for actual D2>
    D3 = D2 + RLFaLbFR <b expanded from previous, actuals need to expand for actual D3>
    D4 = D3 + RLFaRFbRLLFaLbFR <a,b expanded from prevs>
    """
    if n==0:
        return "Fa"
    if n==1:
        return "FaRbFR"
    if n==2:
        return "FaRbFRRLFaLbFR"

    Dprev = "FaRbFRRLFaLbFR"  #D2
    Dn = ''    
    prevexpr = "RbFR"    
    start = 2    
    while start != n:
        start = start + 1
        curexpr = expand(prevexpr)
        Dn = Dprev + expand(curexpr)
        prevexpr = curexpr
        Dprev = Dn
        print "Total Length of D%d : %d" %(start,len(Dn))
        msg = "Iteration:%d,D:%s,Curexpr:%s" % (start,Dn,curexpr)
        f = "d:\\temp\\heigi.txt"
        print "Saving in ",f
        file(f,"w").write(msg)
    return Dn
...
def roboWalk(dn,maxstep=1000000,verbose = False):
    x = 0
    y = 0
    curDir = 'N'
    step = 0
...
def main():
    ...
    x,y = roboWalk(dn,MAX_STEP,verbose)    
